...
def orders(user):
    ...
    base_query = query.order_by(priority_order, WorkOrder.created_at.desc())
...
# 通知函数已抽取到 services/notifier.py（P1e），此处保留转发以兼容旧引用
...
def mobile_shift_handovers(user):
    ...
    # 批量加载所有用户的医院关联（一次查询 user_hospitals，避免每人一次 get_assigned_hospitals）
    from models import user_hospitals as _uh
    if persons:
        uid_list = [p.id for p in persons]
        uh_rows = db.session.query(_uh.c.user_id, _uh.c.hospital_id).filter(
            _uh.c.user_id.in_(uid_list)).all()
        phid_map = {}
        for uid, hid in uh_rows:
            phid_map.setdefault(uid, set()).add(hid)
    else:
        phid_map = {}
    filtered = []
    for p in persons:
        ...
        phids = phid_map.get(p.id) or set()
        if not phids and p.hospital_id:
            phids = {p.hospital_id}
...
def mobile_chat_conversations(user):
    from routes.chat import _serialize_conversations
    ...
    return jsonify({'conversations': _serialize_conversations(conversations, user.id)})
